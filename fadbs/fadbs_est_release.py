...
class EstimateSeriesAniDb(object):
    ...
    def estimate(self, entry, session=None):
        ...
        if any(field in entry for field in ['series_name', 'anidb_title_main', 'anidb_id']):
            series = ANIDB_SEARCH.lookup_series(anidb_id=entry.get('anidb_id'), name=entry.get('anidb_title_main', entry.get('series_name')))
            if series:
